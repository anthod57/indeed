    ...
    <section class="searchbar-container">
        <div v-bind:class="{ searchbar: !searchbarFocused, 'searchbar focused': searchbarFocused }">
            <input
                ...
                v-model="keywords"
                v-on:keyup.enter="searchOffers()"
                v-on:focus="handleSearchbarFocus(true)"
                v-on:blur="handleSearchbarFocus(false)"
            />
...
export default {
    ...
    methods: {
        async searchOffers() {
            ...
            // Pass offers to parent component so we can display them.
            this.$emit("setOffers", data);

            // Store the current search input so it persist in case of refresh.
